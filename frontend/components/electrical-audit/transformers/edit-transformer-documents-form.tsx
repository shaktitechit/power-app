...
interface EditTransformerDocumentsFormProps {
  open: boolean;
...
export function EditTransformerDocumentsForm({
  open,
  onOpenChange,
  onComplete,
  transformerId,
}: EditTransformerDocumentsFormProps) {
  const fileInputRef = useRef<HTMLInputElement | null>(null);

  const { data: transformerResponse, isLoading: transformerLoading } =
    useGetTransformerByIdQuery(transformerId, {
      skip: !transformerId || !open,
    });

  const [updateTransformer, { isLoading: updatingTransformer }] =
    useUpdateTransformerMutation();
  ...
  useEffect(() => {
    if (!transformer || !open) return;

    setExistingDocuments(transformer.documents || []);
    setNewDocuments([]);
    setSubmitError("");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Failed to update Transformer documents:", error);
    }
  };

  const isBusy = updatingTransformer || transformerLoading;
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>Add Documents & Images (Transformer)</DialogTitle>
        </DialogHeader>

        <div className="space-y-6 py-4">
          <div className="rounded-xl border border-dashed p-6">
            <label
              htmlFor="edit_transformer_docs_file"
        ...
        <DialogFooter>
          ...
          >
            {updatingTransformer ? "Uploading..." : "Save Changes"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
