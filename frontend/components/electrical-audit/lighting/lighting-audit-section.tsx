...
} from "@/lib/electrical-audit/lighting-audit-record-excel";
...
export function LightingAuditSection({
  ...
  const { data, isLoading } = useGetLightingAuditsQuery({
    facility_id: facilityId,
    utility_account_id: utilityAccountId,
  });
  ...
  const [deleteTarget, setDeleteTarget] = useState<LightingAuditFormState | null>(
    null,
  );
  ...
  const handleDelete = (form: LightingAuditFormState) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await toastHandler({
        action: () => deleteLightingAudit(deleteTarget.id as string).unwrap(),
      ...
      console.error("Failed to delete lighting audit record:", error);
    }
  };
  ...
  return (
    <div className="relative space-y-4">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete lighting audit record?</AlertDialogTitle>
            <AlertDialogDescription>
