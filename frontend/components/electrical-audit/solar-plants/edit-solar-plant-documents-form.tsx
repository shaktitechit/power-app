...
interface EditSolarPlantDocumentsFormProps {
  open: boolean;
...
export function EditSolarPlantDocumentsForm({
  open,
  onOpenChange,
  onComplete,
  solarPlantId,
}: EditSolarPlantDocumentsFormProps) {
  const fileInputRef = useRef<HTMLInputElement | null>(null);

  const { data: solarResponse, isLoading: solarLoading } =
    useGetSolarPlantByIdQuery(solarPlantId, {
      skip: !solarPlantId || !open,
    });

  const [updateSolarPlant, { isLoading: updatingSolar }] =
    useUpdateSolarPlantMutation();

  const solarPlant = solarResponse?.data;
  ...
  useEffect(() => {
    if (!solarPlant || !open) return;

    setExistingDocuments(solarPlant.documents || []);
    setNewDocuments([]);
    setSubmitError("");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Failed to update Solar Plant documents:", error);
    }
  };

  const isBusy = updatingSolar || solarLoading;
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>Add Documents & Images (Solar Plant)</DialogTitle>
        </DialogHeader>

        <div className="space-y-6 py-4">
          <div className="rounded-xl border border-dashed p-6">
            <label
              htmlFor="edit_solar_docs_file"
        ...
        <DialogFooter>
          ...
          >
            {updatingSolar ? "Uploading..." : "Save Changes"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
