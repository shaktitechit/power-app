...
import {
  ...
  parseFanAuditExcel,
} from "@/lib/electrical-audit/fan-audit-record-excel";
...
export function FanAuditRecordSection({
  ...
  const [deleteTarget, setDeleteTarget] = useState<FanAuditFormState | null>(null);
  ...
  const handleDelete = (form: FanAuditFormState) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await toastHandler({
        action: () => deleteFanAuditRecord(deleteTarget.id as string).unwrap(),
      ...
      console.error("Failed to delete fan audit record:", error);
    }
  };
