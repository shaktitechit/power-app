...
type AuditStepSubmitBarProps = {
  ...
  stepLabel?: string;
  /** When true, this audit step is already submitted (locked). Non-permitted users see nothing. */
...
export function AuditStepSubmitBar({
  ...
  const canAllowAuditStep = canManageResource(
    user?.role,
    user?.permissions || [],
    "utility_audit_flow",
    "allow_audit_step",
  ...
  if (auditStepLocked && !canAllowAuditStep) {
    if (variant === "compact") {
      return null;
    }
