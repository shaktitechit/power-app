...
} from "@/store/slices/safety-audit/safetyMeteringRoomAuditApiSlice";
...
const SAFETY_AUDIT_TAB_ID = "metering-room";

export interface SafetyMeteringRoomSectionProps {
  facilityId: string;
...
type SafetyMeteringRoomFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
/** Default metering room safety checklist (activity / observation + requirement). */
const DEFAULT_METERING_ROOM_CHECKLIST_SPEC: readonly {
  activity_description: string;
  requirement: string;
}[] = [
  {
    activity_description:
      "Check the incomers are correctly identified",
    requirement: "Visual Marking",
  },
  {
    activity_description:
      "In case of two incomers check correct interlock is provided for safe operation.",
    requirement: "Suitable electrical or mechanical interlock.",
  },
  {
    activity_description: "Check pad locking provision for incomers",
    requirement: "Lockout–Tagout (LOTO) policy provisions",
  },
  {
    activity_description:
      "Check protection provision in incomer MCCB",
    requirement: "Overcurrent, short circuit protection.",
  },
  {
    activity_description:
      "Check site power factor control and monitoring provision.",
    requirement: "APFC panel. Note power factor",
  },
  {
    activity_description:
      "Check earthing provision in metering room equipment.",
    requirement: "Two-point earthing",
  },
  {
    activity_description: "CT ratio match",
    requirement: "As per OEM & single line diagram",
  },
];

function defaultMeteringRoomChecklistRows(): ChecklistRow[] {
  return DEFAULT_METERING_ROOM_CHECKLIST_SPEC.map((row, i) => ({
    localKey: nextKey(),
    sr_no: i + 1,
...
const createEmptyForm = (): SafetyMeteringRoomFormState => ({
  localId: `new-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
  isNew: true,
  isEditing: true,
  audit_date: "",
  status: "draft",
  items: defaultMeteringRoomChecklistRows(),
  documents: [],
  existingDocuments: [],
});
...
function recordToForm(
  ...
  return {
    ...
    status:
      (r.status as SafetyMeteringRoomFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — metering room (`SafetyMeteringRoomAudit`). Single record per utility account in the UI. */
export function SafetyMeteringRoomSection({
  facilityId,
  utilityAccountId,
  auditStepLocked = false,
  auditStepNoData,
}: SafetyMeteringRoomSectionProps) {
  ...
  /** Only the newest record is shown/edited; API may still return multiple rows. */
  const serverRecord = audits[0];

  const [form, setForm] = useState<SafetyMeteringRoomFormState | null>(null);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyMeteringRoomFormState | null>(null);
  ...
  const patchForm = (
    up: (f: SafetyMeteringRoomFormState) => SafetyMeteringRoomFormState,
  ) => {
    setForm((f) => (f ? up(f) : f));
  };
  ...
  const handleCancel = (f: SafetyMeteringRoomFormState) => {
    if (f.isNew) {
      setForm(null);
      return;
    }
  ...
  const handleSave = async (form: SafetyMeteringRoomFormState) => {
    setBackendError("");
    ...
    try {
      await toastHandler({
        ...
        success: form.isNew
          ? "Metering room safety audit created"
          : "Saved",
      });
  ...
  const onDelete = (f: SafetyMeteringRoomFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load metering room safety audits.{" "}
  ...
  if (listLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading metering room safety audits…
      </div>
    );
  }
    ...
    <div className="relative space-y-4">
      ...
      {audits.length > 1 && (
        <p className="text-xs text-muted-foreground">
          Multiple metering room records exist for this account; only the newest
          is shown and edited here.
        </p>
      )}
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      patchForm((f) => ({
                        ...f,
                        status: e.target
                          .value as SafetyMeteringRoomFormState["status"],
                      }))
                    }
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this metering room safety audit. This
