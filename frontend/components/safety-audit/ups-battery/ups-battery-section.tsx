...
import {
  useCreateSafetyUpsAuditMutation,
  useDeleteSafetyUpsAuditMutation,
  useGetSafetyUpsAuditsQuery,
...
const SAFETY_AUDIT_TAB_ID = "ups-battery";

export interface SafetyUpsBatterySectionProps {
  facilityId: string;
...
type SafetyUpsBatteryFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
function toOptionalKva(s: string): number | undefined {
  const t = s.trim();
...
/** Default UPS & battery safety checklist (activity + requirement). */
const DEFAULT_UPS_BATTERY_CHECKLIST_SPEC: readonly {
  activity_description: string;
  requirement: string;
}[] = [
  {
    activity_description:
      "Check provision for battery rack earthing.",
    requirement:
      "Visual inspection.",
  },
  {
    activity_description:
      "Check cell identification.",
    requirement:
      "Visual inspection.",
  },
  {
  ...
  {
    activity_description:
      "Check terminal for rusting or corrosive material deposited.",
    requirement:
      "Visual inspection.",
  },
  {
    activity_description:
      "Check for isolation at battery end.",
    requirement:
      "Visual inspection.",
  },
  {
    activity_description:
      "Check any damage or bulge condition.",
    requirement:
      "Visual inspection.",
  },
  {
    activity_description:
      "Check eye washer wherever lead acid batteries are installed.",
    requirement:
      "Visual inspection.",
  },
];

function defaultUpsBatteryChecklistRows(): ChecklistRow[] {
  return DEFAULT_UPS_BATTERY_CHECKLIST_SPEC.map((row, i) => ({
    localKey: nextKey(),
    sr_no: i + 1,
...
const createEmptyForm = (): SafetyUpsBatteryFormState => ({
  ...
  audit_date: "",
  status: "draft",
  items: defaultUpsBatteryChecklistRows(),
  documents: [],
  existingDocuments: [],
});
...
function recordToForm(
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    ups_name: asFormInputString(raw.ups_name),
    capacity_kva: asFormInputString(raw.capacity_kva),
    location: asFormInputString(raw.location),
    audit_date: toDateInput(r.audit_date),
    status: (r.status as SafetyUpsBatteryFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — UPS & battery checklist (`SafetyUpsAudit`). */
export function SafetyUpsBatterySection({
  facilityId,
  utilityAccountId,
  auditStepLocked = false,
  auditStepNoData,
}: SafetyUpsBatterySectionProps) {
  ...
  const [forms, setForms] = useState<SafetyUpsBatteryFormState[]>([]);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyUpsBatteryFormState | null>(null);
  ...
  const updateForm = (
    localId: string,
    up: (f: SafetyUpsBatteryFormState) => SafetyUpsBatteryFormState,
  ) => {
  ...
  const replaceForm = (
    localId: string,
    next: SafetyUpsBatteryFormState,
  ) => {
  ...
  const handleCancel = (form: SafetyUpsBatteryFormState) => {
    if (form.isNew) {
  ...
  const handleSave = async (form: SafetyUpsBatteryFormState) => {
    setBackendError("");
    ...
    const payload = {
      ...
      ups_name: form.ups_name.trim() || undefined,
      capacity_kva: toOptionalKva(form.capacity_kva),
    ...
    try {
      await toastHandler({
        ...
        success: form.isNew
          ? "UPS & battery safety audit created"
          : "Saved",
      });
      await refetch();
    } catch (err) {
  ...
  const onDelete = (f: SafetyUpsBatteryFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load UPS & battery safety audits.{" "}
  ...
  if (listLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading UPS & battery safety audits…
      </div>
    );
  }
          ...
          <Card key={form.localId}>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>
                ...
                {form.isNew ? " (new)" : ""}
                {typeof form.ups_name === "string" && form.ups_name
                  ? ` — ${form.ups_name}`
                  : ""}
              </CardTitle>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        ups_name: e.target.value,
                      }))
                    }
                    readOnly={!form.isEditing || auditStepLocked}
                    placeholder="e.g. UPS-IDF-2"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    readOnly={!form.isEditing || auditStepLocked}
                    placeholder="Room / rack reference"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        status: e.target
                          .value as SafetyUpsBatteryFormState["status"],
                      }))
                    }
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this UPS & battery safety audit. This
