...
} from "@/store/slices/safety-audit/safetyDocumentsAuditApiSlice";
...
const SAFETY_AUDIT_TAB_ID = "documents-review";

export interface SafetyDocumentsReviewSectionProps {
  facilityId: string;
...
type SafetyDocumentsReviewFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
  /** Name or asset tag (matches `equipment_name` on server). */
...
function formatAuditDateLabel(isoDay: string): string {
  if (!isoDay?.trim()) return "No audit date";
  const d = new Date(isoDay + "T12:00:00");
  if (Number.isNaN(d.getTime())) return "No audit date";
...
const DEFAULT_DOCUMENTS_REVIEW_CHECKLIST_SPEC: readonly {
  activity_description: string;
  requirement: string;
}[] = [
  {
    activity_description: "Documentation",
    requirement: "Safety manuals, SOPs",
  },
  {
    activity_description: "PPE",
    requirement: "Inventory, condition logs",
  },
  {
    activity_description: "Shock treatment chart displayed at panels",
    requirement:
      "Chart in Hindi + local language, laminated, visible",
  },
  {
    activity_description: "Chart content accuracy",
    requirement:
      "Includes first aid, CPR steps, emergency contacts",
  },
  {
    activity_description: "Training records available",
    requirement:
      "Electrical safety & first aid logs maintained annually",
  },
  {
    activity_description: "Check single line diagram",
    requirement: "Approved / updated",
  },
  {
    activity_description:
      "Verify all the components marked in SLD are available physically and are of correctly marked ratings",
    requirement: "Approved / updated",
  },
  {
    activity_description: "Check as built schemes are available",
    requirement: "Approved / updated",
  },
  {
    activity_description: "All pits uniquely tagged",
    requirement: "Tags attached and mapped",
  },
  {
    activity_description: "Lightning earth isolated",
    requirement:
      "Separation from electrical earth confirmed",
  },
  {
    activity_description: "Check earth pit test records",
    requirement: "Not older than 6 months",
  },
  {
    activity_description: "Check maintenance schedule",
    requirement: "Approved / updated",
  },
  {
    activity_description: "Check maintenance check lists",
    requirement: "With respect to schedule",
  },
  {
    activity_description: "Check for instrument calibration records",
    requirement: "From NABL approved lab.",
  },
  {
    activity_description:
      "Check for electrical accident/ incident records and investigation reports",
    requirement: "With corrective action",
  },
  {
    activity_description: "Check for thermography reports",
    requirement: "With corrective action if any.",
  },
  {
    activity_description:
      "Check for cable and equipment IR test reports",
    requirement: "Last 3 test data",
  },
  {
    activity_description:
      "AMC for major electrical equipment",
    requirement:
      "DG/ Compressor/ elevators/UPS/Batteries.",
  },
  {
    activity_description:
      "Transformer oil test records available (Break Down Voltage- BDV and Dissolved Gas Analysis-DGA)",
    requirement:
      "Last 3-year test reports (to be carried out at least once in a year)",
  },
  {
    activity_description: "ACB-Panels",
    requirement: "Trip Test, Timing",
  },
  {
    activity_description: "VCB-Main",
    requirement: "Primary Injection",
  },
  {
    activity_description:
      "Records of electrical safety training",
    requirement: "Check last 2-year reports",
  },
  {
    activity_description:
      "Reports of inspection of electrical installations by local authorities",
    requirement: "Check last 3-year reports",
  },
  {
    activity_description:
      "Calibration certificates for electrical measuring instruments",
    requirement: "Check last 3-year reports",
  },
  {
    activity_description:
      "Test certificates for rubber insulating gloves (for electrical purpose)",
    requirement:
      "Check last 3-year reports (to be carried out at least once in 6 months)",
  },
];

function defaultDocumentsReviewChecklistRows(): ChecklistRow[] {
  return DEFAULT_DOCUMENTS_REVIEW_CHECKLIST_SPEC.map((row, i) => ({
    localKey: nextKey(),
    sr_no: i + 1,
...
const createEmptyForm = (): SafetyDocumentsReviewFormState => ({
  ...
  audit_date: "",
  status: "draft",
  items: defaultDocumentsReviewChecklistRows(),
  documents: [],
  existingDocuments: [],
});
...
function recordToForm(
...
): SafetyDocumentsReviewFormState {
  const items =
    Array.isArray(r.items) && r.items.length > 0
      ? (r.items as SafetyChecklistItem[]).map((it, i) => itemToRow(it, i))
      : defaultDocumentsReviewChecklistRows();
  ...
  return {
    ...
    status:
      (r.status as SafetyDocumentsReviewFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — documents review (`SafetyDocumentsAudit`). Multiple records per utility account. */
export function SafetyDocumentsReviewSection({
  facilityId,
  utilityAccountId,
  auditStepLocked = false,
  auditStepNoData,
}: SafetyDocumentsReviewSectionProps) {
  ...
  const [forms, setForms] = useState<SafetyDocumentsReviewFormState[]>([]);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyDocumentsReviewFormState | null>(null);
  ...
  const updateForm = (
    localId: string,
    up: (f: SafetyDocumentsReviewFormState) => SafetyDocumentsReviewFormState,
  ) => {
  ...
  const replaceForm = (
    localId: string,
    next: SafetyDocumentsReviewFormState,
  ) => {
  ...
  const handleCancel = (form: SafetyDocumentsReviewFormState) => {
    if (form.isNew) {
  ...
  const handleSave = async (form: SafetyDocumentsReviewFormState) => {
    setBackendError("");
    ...
    try {
      await toastHandler({
        ...
        success: form.isNew ? "Documents review created" : "Saved",
      });
      await refetch();
    } catch (err) {
  ...
  const onDelete = (f: SafetyDocumentsReviewFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load document review records. {getErrorMessage(listError)}
        </p>
        <Button
          type="button"
          variant="outline"
  ...
  if (listLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading document review records…
      </div>
    );
  }
            ...
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>
                ...
                {form.equipment_name.trim() ||
                  form.location.trim() ||
                  formatAuditDateLabel(form.audit_date)}
              </CardTitle>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g. Main LT panel bundle"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        status: e.target
                          .value as SafetyDocumentsReviewFormState["status"],
                      }))
                    }
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this documents review. This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
