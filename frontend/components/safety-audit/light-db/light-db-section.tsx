...
} from "@/store/slices/safety-audit/safetyLdbAuditApiSlice";
...
const SAFETY_AUDIT_TAB_ID = "light-db";

export interface SafetyLightDbSectionProps {
  facilityId: string;
...
/** Default light distribution board (Light DB) safety checklist (activity + requirement). */
const DEFAULT_LIGHT_DB_CHECKLIST_SPEC: readonly {
  activity_description: string;
  requirement: string;
}[] = [
  {
    activity_description:
      "Check provision for protection against earth leakage currents to ensure personnel safety and minimize electrical hazards.",
    requirement:
      "RCD/ELCB provided with 30 mA rating.",
  },
  {
    activity_description:
      "Check spare MCB cutouts.",
    requirement:
      "All unused cutouts properly closed.",
  },
  {
    activity_description:
      "Check for circuit identification.",
    requirement:
      "Identification nameplates provided and legible.",
  },
  {
    activity_description:
      "Check earthing provision at LDB.",
    requirement:
      "Two-point earthing provided.",
  },
];

function defaultLightDbChecklistRows(): ChecklistRow[] {
  return DEFAULT_LIGHT_DB_CHECKLIST_SPEC.map((row, i) => ({
    localKey: nextKey(),
    sr_no: i + 1,
...
const createEmptyForm = (): SafetyLightDbFormState => ({
  ...
  audit_date: "",
  status: "draft",
  items: defaultLightDbChecklistRows(),
  documents: [],
  existingDocuments: [],
});
...
function recordToForm(
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    ldb_name: asFormInputString(raw.ldb_name),
    location: asFormInputString(raw.location),
    audit_date: toDateInput(r.audit_date),
    status: (r.status as SafetyLightDbFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — Light DB checklist (`SafetyLdbAudit`). */
export function SafetyLightDbSection({
  facilityId,
  utilityAccountId,
  auditStepLocked = false,
  auditStepNoData,
}: SafetyLightDbSectionProps) {
  ...
  const [forms, setForms] = useState<SafetyLightDbFormState[]>([]);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyLightDbFormState | null>(null);
  ...
  const updateForm = (
    localId: string,
    up: (f: SafetyLightDbFormState) => SafetyLightDbFormState,
  ) => {
  ...
  const handleCancel = (form: SafetyLightDbFormState) => {
    if (form.isNew) {
  ...
  const handleSave = async (form: SafetyLightDbFormState) => {
    setBackendError("");
    ...
    const payload = {
      ...
      ldb_name: form.ldb_name.trim() || undefined,
  ...
  const onDelete = (f: SafetyLightDbFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load Light DB safety audits.{" "}
  ...
  if (listLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading Light DB safety audits…
      </div>
    );
  }
          ...
          <Card key={form.localId}>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>
                ...
                {form.isNew ? " (new)" : ""}
                {typeof form.ldb_name === "string" && form.ldb_name
                  ? ` — ${form.ldb_name}`
                  : ""}
              </CardTitle>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        ldb_name: e.target.value,
                      }))
                    }
                    readOnly={!form.isEditing || auditStepLocked}
                    placeholder="e.g. LTP 3F Lighting DB"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        status: e.target
                          .value as SafetyLightDbFormState["status"],
                      }))
                    }
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this Light DB safety audit. This
