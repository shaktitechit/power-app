...
import {
  useCreateSafetyPumpCompressorAuditMutation,
  useDeleteSafetyPumpCompressorAuditMutation,
  useGetSafetyPumpCompressorAuditsQuery,
  useUpdateSafetyPumpCompressorAuditMutation,
} from "@/store/slices/safety-audit/safetyPumpCompressorAuditApiSlice";
...
const SAFETY_AUDIT_TAB_ID = "pump-compressor";

export interface SafetyPumpCompressorSectionProps {
  facilityId: string;
...
const EQUIPMENT_TYPES = ["pump", "compressor", "motor", "other"] as const;
...
type SafetyPumpCompressorFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
const DEFAULT_PUMP_COMPRESSOR_CHECKLIST_SPEC: readonly {
  activity_description: string;
  requirement: string;
}[] = [
  {
    activity_description: "Check ventilation provision for area.",
    requirement: "Proper ventilation",
  },
  {
    activity_description: "Check pump house motors are with correct overload protection.",
    requirement: "OLR relay setting.",
  },
  {
    activity_description: "Check motors IP protection",
    requirement: "IP55",
  },
  {
    activity_description: "Check earthing provision in pump room equipment.",
    requirement: "Two-point earthing",
  },
];

function defaultPumpCompressorChecklistRows(): ChecklistRow[] {
  return DEFAULT_PUMP_COMPRESSOR_CHECKLIST_SPEC.map((row, i) => ({
    localKey: nextKey(),
    sr_no: i + 1,
...
const createEmptyForm = (): SafetyPumpCompressorFormState => ({
  ...
  audit_date: "",
  status: "draft",
  items: defaultPumpCompressorChecklistRows(),
  documents: [],
  existingDocuments: [],
});
...
function recordToForm(
...
): SafetyPumpCompressorFormState {
  const items =
    Array.isArray(r.items) && r.items.length > 0
      ? (r.items as SafetyChecklistItem[]).map((it, i) => itemToRow(it, i))
      : defaultPumpCompressorChecklistRows();
  ...
  return {
    ...
    status: (r.status as SafetyPumpCompressorFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — pumps & compressors (`SafetyPumpCompressorAudit`). Multiple records per utility account. */
export function SafetyPumpCompressorSection({
  facilityId,
  utilityAccountId,
  auditStepLocked = false,
  auditStepNoData,
}: SafetyPumpCompressorSectionProps) {
  ...
  const [forms, setForms] = useState<SafetyPumpCompressorFormState[]>([]);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyPumpCompressorFormState | null>(null);
  ...
  const updateForm = (
    localId: string,
    up: (f: SafetyPumpCompressorFormState) => SafetyPumpCompressorFormState,
  ) => {
  ...
  const replaceForm = (
    localId: string,
    next: SafetyPumpCompressorFormState,
  ) => {
  ...
  const handleCancel = (form: SafetyPumpCompressorFormState) => {
    if (form.isNew) {
  ...
  const handleSave = async (form: SafetyPumpCompressorFormState) => {
    setBackendError("");
    ...
    try {
      await toastHandler({
        ...
        success: form.isNew ? "Pump / compressor audit created" : "Saved",
      });
      await refetch();
    } catch (err) {
  ...
  const onDelete = (f: SafetyPumpCompressorFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load pump / compressor audits. {getErrorMessage(listError)}
        </p>
        <Button
          type="button"
          variant="outline"
  ...
  if (listLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading pump / compressor audits…
      </div>
    );
  }
            ...
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>
                ...
                {form.equipment_name.trim() ||
                  EQUIPMENT_TYPE_OPTIONS.find((o) => o.value === form.equipment_type)
                    ?.label ||
                  "Equipment"}
              </CardTitle>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        status: e.target
                          .value as SafetyPumpCompressorFormState["status"],
                      }))
                    }
                    disabled={!form.isEditing || auditStepLocked}
                  >
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this pump / compressor audit. This
