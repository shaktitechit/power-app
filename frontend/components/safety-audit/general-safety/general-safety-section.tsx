...
type SafetyGeneralSafetyFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
/** Default general electrical safety checklist (activity + requirement). */
...
function recordToForm(
  ...
  return {
    ...
    status:
      (r.status as SafetyGeneralSafetyFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — general safety checklist (`SafetyGeneralAudit`). */
export function SafetyGeneralSafetySection({
  ...
  /** One general-safety audit per utility account in the UI; API may return more than one — we use the newest only. */
  const serverRecord = audits[0];

  const [form, setForm] = useState<SafetyGeneralSafetyFormState | null>(null);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyGeneralSafetyFormState | null>(null);
  ...
  const patchForm = (
    up: (f: SafetyGeneralSafetyFormState) => SafetyGeneralSafetyFormState,
  ) => {
    setForm((f) => (f ? up(f) : f));
  };
  ...
  const handleCancel = (f: SafetyGeneralSafetyFormState) => {
    if (f.isNew) {
      setForm(null);
      return;
    }
  ...
  const handleSave = async (form: SafetyGeneralSafetyFormState) => {
    setBackendError("");
  ...
  const onDelete = (f: SafetyGeneralSafetyFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load general safety audits.{" "}
    ...
    <div className="relative space-y-4">
      ...
      {audits.length > 1 && (
        <p className="text-xs text-muted-foreground">
          Multiple records exist for this account; only the newest is shown and
          edited here.
        </p>
      )}
                ...
                <select
                  ...
                  onChange={(e) =>
                    patchForm((f) => ({
                      ...f,
                      status: e.target
                        .value as SafetyGeneralSafetyFormState["status"],
                    }))
                  }
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              updateRow(row.localKey, {
                                compliance: e.target.value as SafetyCompliance,
                              })
                            }
                            disabled={!form.isEditing || auditStepLocked}
                          >
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this general safety audit. This
