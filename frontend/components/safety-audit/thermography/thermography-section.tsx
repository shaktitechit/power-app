...
import {
  useCreateSafetyThermographyAuditMutation,
  useDeleteSafetyThermographyAuditMutation,
  useGetSafetyThermographyAuditsQuery,
  useUpdateSafetyThermographyAuditMutation,
} from "@/store/slices/safety-audit/safetyThermographyAuditApiSlice";
...
const SAFETY_AUDIT_TAB_ID = "thermography";

export interface SafetyThermographySectionProps {
  facilityId: string;
...
type SafetyThermographyFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
  ...
  delta_temperature_c: string;
  urgency: ThermographyUrgency;
  /** Saved pairs from API — display only; uploads not edited here. */
  savedImages: ThermographyImagePair[];
  audit_date: string;
...
const URGENCY_OPTIONS: { value: ThermographyUrgency; label: string }[] = [
  { value: "", label: "—" },
...
function normalizeUrgency(v: unknown): ThermographyUrgency {
  const s = typeof v === "string" ? v : "";
  const ok = ["normal", "medium", "high", "critical", ""].includes(s);
  return ok ? (s as ThermographyUrgency) : "";
}

function normalizeSavedImages(raw: unknown): ThermographyImagePair[] {
  if (!Array.isArray(raw)) return [];
  const out: ThermographyImagePair[] = [];
  for (const row of raw) {
    if (!row || typeof row !== "object") continue;
    const r = row as Record<string, unknown>;
    out.push({
      thermal_image_url:
        typeof r.thermal_image_url === "string"
          ? r.thermal_image_url
          : undefined,
      visible_image_url:
        typeof r.visible_image_url === "string"
          ? r.visible_image_url
          : undefined,
...
const DEFAULT_THERMO_CHECKLIST_SPEC: readonly {
  activity_description: string;
  requirement: string;
}[] = [
  {
    activity_description: "Thermography done live",
    requirement: "With PPE and trained operator",
  },
  {
    activity_description: "ΔT within safe limits",
    requirement: "≤ 20 °C ideal",
  },
  {
    activity_description: "Critical hotspots flagged",
    requirement: "> 40 °C = high urgency",
  },
  {
    activity_description: "Photos captured and logged",
    requirement: "Thermal and visible images captured",
  },
];

function defaultThermoChecklistRows(): ChecklistRow[] {
  return DEFAULT_THERMO_CHECKLIST_SPEC.map((row, i) => ({
    localKey: nextKey(),
    sr_no: i + 1,
...
const createEmptyForm = (): SafetyThermographyFormState => ({
  ...
  audit_date: "",
  status: "draft",
  items: defaultThermoChecklistRows(),
  documents: [],
  existingDocuments: [],
});
...
function recordToForm(
  ...
  const insp = raw.inspection_date ?? r.audit_date;
  const inspection_date =
    typeof insp === "string" || insp instanceof Date
      ? toDateInput(
          typeof insp === "string"
            ? insp
            : insp instanceof Date
              ? insp.toISOString()
              : "",
        )
      : "";
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    inspection_date: inspection_date || toDateInput(r.audit_date),
    inspected_by: asFormInputString(raw.inspected_by),
    location: asFormInputString(raw.location),
    max_temperature_c: numToInput(raw.max_temperature_c),
    delta_temperature_c: numToInput(raw.delta_temperature_c),
    urgency: normalizeUrgency(raw.urgency),
    savedImages: normalizeSavedImages(raw.images),
    audit_date: toDateInput(r.audit_date),
    status: (r.status as SafetyThermographyFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — thermography (`SafetyThermographyAudit`). Multiple records per utility account. */
export function SafetyThermographySection({
  facilityId,
  utilityAccountId,
  auditStepLocked = false,
  auditStepNoData,
}: SafetyThermographySectionProps) {
  ...
  const [forms, setForms] = useState<SafetyThermographyFormState[]>([]);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyThermographyFormState | null>(null);
  ...
  const updateForm = (
    localId: string,
    up: (f: SafetyThermographyFormState) => SafetyThermographyFormState,
  ) => {
  ...
  const replaceForm = (
    localId: string,
    next: SafetyThermographyFormState,
  ) => {
  ...
  const handleCancel = (form: SafetyThermographyFormState) => {
    if (form.isNew) {
  ...
  const buildNumericAndMetaPayload = (form: SafetyThermographyFormState) => {
    const max_temperature_c = optionalParsedNumber(form.max_temperature_c);
    const delta_temperature_c = optionalParsedNumber(form.delta_temperature_c);
    return {
      inspection_date: form.inspection_date || undefined,
      inspected_by: form.inspected_by.trim() || undefined,
      location: form.location.trim() || undefined,
      ...(max_temperature_c !== undefined ? { max_temperature_c } : {}),
      ...(delta_temperature_c !== undefined ? { delta_temperature_c } : {}),
      ...(form.urgency ? { urgency: form.urgency } : { urgency: "" }),
    };
  };

  const handleSave = async (form: SafetyThermographyFormState) => {
    setBackendError("");
    ...
    const payload = {
      ...
      items,
      ...buildNumericAndMetaPayload(form),
    ...
    try {
      await toastHandler({
        ...
        success: form.isNew ? "Thermography audit created" : "Saved",
      });
      await refetch();
    } catch (err) {
  ...
  const onDelete = (f: SafetyThermographyFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load thermography audits. {getErrorMessage(listError)}
        </p>
        <Button
          type="button"
          variant="outline"
        ...
        forms.map((form, index) => (
          <Card key={form.localId}>
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                  ...
                  <Input
                    className={inputClass(form.isEditing && !auditStepLocked)}
                    inputMode="decimal"
                    value={form.max_temperature_c}
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        max_temperature_c: e.target.value,
                      }))
                    }
                    readOnly={!form.isEditing || auditStepLocked}
                  />
                ...
                <div>
                  ...
                  <Input
                    className={inputClass(form.isEditing && !auditStepLocked)}
                    inputMode="decimal"
                    value={form.delta_temperature_c}
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        delta_temperature_c: e.target.value,
                      }))
                    }
                    readOnly={!form.isEditing || auditStepLocked}
                    placeholder="Vs reference"
                  />
                </div>
                <div>
                  <Label>Urgency</Label>
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        urgency: e.target.value as ThermographyUrgency,
                      }))
                    }
                    disabled={!form.isEditing || auditStepLocked}
                  >
                    {URGENCY_OPTIONS.map((o) => (
                      <option key={o.value || "urgency-empty"} value={o.value}>
                        {o.label}
                      </option>
                    ))}
                  </select>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        status: e.target
                          .value as SafetyThermographyFormState["status"],
                      }))
                    }
                    disabled={!form.isEditing || auditStepLocked}
                  >
              ...
              {form.savedImages.length > 0 && (
                <div className="rounded-xl border p-4">
                  <h4 className="mb-2 text-sm font-semibold text-foreground">
                    Saved thermal / visible pairs (read-only)
                  </h4>
                  <p className="mb-3 text-xs text-muted-foreground">
                    Image pairs are loaded from the server. Manage uploads through
                    backend or a dedicated flow if required.
                  </p>
                  <ul className="space-y-3 text-sm">
                    {form.savedImages.map((pair, i) => (
                      <li
                        key={`pair-${i}-${pair.thermal_image_url ?? pair.visible_image_url ?? ""}`}
                        className="rounded-md border border-border/80 p-3"
                      >
                        <div className="flex flex-wrap gap-3">
                          {pair.thermal_image_url ? (
                            <a
                              href={pair.thermal_image_url}
                              target="_blank"
                              rel="noreferrer"
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this thermography audit. This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
