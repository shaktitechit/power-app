...
} from "@/store/slices/safety-audit/safetyElevatorAuditApiSlice";
...
const SAFETY_AUDIT_TAB_ID = "elevator-safety";

export interface SafetyElevatorSafetySectionProps {
  facilityId: string;
...
type SafetyElevatorSafetyFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
/** Default elevator safety checklist (`SafetyElevatorAudit` items schema). */
const DEFAULT_ELEVATOR_CHECKLIST_SPEC: readonly {
  activity_description: string;
  requirement: string;
}[] = [
  {
    activity_description: "Door Safety Sensor",
    requirement: "Obstruction stops door closure",
  },
  {
    activity_description: "Door Interlock",
    requirement: "Cannot open landing door without car",
  },
  {
    activity_description: "Car Alarm",
    requirement: "Alarm works from inside cab",
  },
  {
    activity_description: "Emergency Stop",
    requirement: "Car halts on pressing red button",
  },
  {
    activity_description: "Fireman’s Switch",
    requirement: "Sends car to ground, disables controls",
  },
  {
    activity_description: "Shaft Wiring",
    requirement: "Properly enclosed & routed",
  },
  {
    activity_description: "Machine Room Wiring",
    requirement: "Labelled, safe, separate power/control",
  },
  {
    activity_description: "Extinguisher & Chart",
    requirement: "CO₂/DCP & first aid info present",
  },
  {
    activity_description: "Safety test Certificate displayed in cab",
    requirement: "Visual Inspection",
  },
  {
    activity_description: "Emergency phone numbers are displayed in the cab and are working",
    requirement: "Visual Inspection",
  },
];

function defaultElevatorChecklistRows(): ChecklistRow[] {
  return DEFAULT_ELEVATOR_CHECKLIST_SPEC.map((row, i) => ({
    localKey: nextKey(),
    sr_no: i + 1,
...
const createEmptyForm = (): SafetyElevatorSafetyFormState => ({
  ...
  audit_date: "",
  status: "draft",
  items: defaultElevatorChecklistRows(),
  documents: [],
  existingDocuments: [],
});
...
function recordToForm(
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    elevator_name: asFormInputString(raw.elevator_name),
    location: asFormInputString(raw.location),
    audit_date: toDateInput(r.audit_date),
    status: (r.status as SafetyElevatorSafetyFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — elevator (`SafetyElevatorAudit`). Multiple records per utility account. */
export function SafetyElevatorSafetySection({
  facilityId,
  utilityAccountId,
  auditStepLocked = false,
  auditStepNoData,
}: SafetyElevatorSafetySectionProps) {
  ...
  const [forms, setForms] = useState<SafetyElevatorSafetyFormState[]>([]);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyElevatorSafetyFormState | null>(null);
  ...
  const updateForm = (
    localId: string,
    up: (f: SafetyElevatorSafetyFormState) => SafetyElevatorSafetyFormState,
  ) => {
  ...
  const replaceForm = (
    localId: string,
    next: SafetyElevatorSafetyFormState,
  ) => {
  ...
  const handleCancel = (form: SafetyElevatorSafetyFormState) => {
    if (form.isNew) {
  ...
  const handleSave = async (form: SafetyElevatorSafetyFormState) => {
    setBackendError("");
    ...
    const payload = {
      ...
      elevator_name: form.elevator_name.trim() || undefined,
  ...
  const onDelete = (f: SafetyElevatorSafetyFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load elevator safety audits.{" "}
  ...
  if (listLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading elevator safety audits…
      </div>
    );
  }
          ...
          <Card key={form.localId}>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>
                ...
                {form.isNew ? " (new)" : ""}
                {form.elevator_name.trim() ? ` — ${form.elevator_name}` : ""}
              </CardTitle>
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <Label>Elevator name / tag</Label>
                  <Input
                    className={inputClass(form.isEditing && !auditStepLocked)}
                    value={form.elevator_name}
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        elevator_name: e.target.value,
                      }))
                    }
                    readOnly={!form.isEditing || auditStepLocked}
                    placeholder="e.g. Lift-01 / Passenger A"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        status: e.target
                          .value as SafetyElevatorSafetyFormState["status"],
                      }))
                    }
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this elevator safety audit. This
