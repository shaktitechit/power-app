...
type SafetyAdditionalItemsFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
function recordToForm(
...
): SafetyAdditionalItemsFormState {
  const items =
  ...
  return {
    ...
    status:
      (r.status as SafetyAdditionalItemsFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — additional items (`SafetyAdditionalItemsAudit`). Multiple records per utility account. */
export function SafetyAdditionalItemsSection({
  ...
  const [forms, setForms] = useState<SafetyAdditionalItemsFormState[]>([]);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyAdditionalItemsFormState | null>(null);
  ...
  const updateForm = (
    localId: string,
    up: (f: SafetyAdditionalItemsFormState) => SafetyAdditionalItemsFormState,
  ) => {
  ...
  const replaceForm = (
    localId: string,
    next: SafetyAdditionalItemsFormState,
  ) => {
  ...
  const handleCancel = (form: SafetyAdditionalItemsFormState) => {
    if (form.isNew) {
  ...
  const handleSave = async (form: SafetyAdditionalItemsFormState) => {
    setBackendError("");
    ...
    try {
      await toastHandler({
        ...
        success: form.isNew ? "Additional items audit created" : "Saved",
      });
      await refetch();
    } catch (err) {
  ...
  const onDelete = (f: SafetyAdditionalItemsFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load additional items audits.{" "}
  ...
  if (listLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading additional items audits…
      </div>
    );
  }
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g. Annex / roof / yard"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        status: e.target
                          .value as SafetyAdditionalItemsFormState["status"],
                      }))
                    }
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this additional items audit. This
