...
} from "@/store/slices/safety-audit/safetyPanelRoomAuditApiSlice";
...
const SAFETY_AUDIT_TAB_ID = "panel-room";

export interface SafetyPanelRoomSectionProps {
  facilityId: string;
...
const PANEL_TYPES = ["pdb", "mdb", "sub_panel", "other"] as const;
type PanelType = (typeof PANEL_TYPES)[number];

type SafetyPanelRoomFormState = {
  id?: string;
  localId: string;
  isNew: boolean;
  isEditing: boolean;
...
function normalizePanelType(v: unknown): PanelType {
  const s = typeof v === "string" ? v : "";
  return PANEL_TYPES.includes(s as PanelType) ? (s as PanelType) : "pdb";
}
...
/** Default panel room safety checklist (activity / observation + requirement). */
const DEFAULT_PANEL_ROOM_CHECKLIST_SPEC: readonly {
  activity_description: string;
  requirement: string;
}[] = [
  {
    activity_description:
      "Verify panel identification of PDB.",
    requirement:
      "Title/nameplate clearly visible and legible.",
  },
  {
    activity_description:
      "Check load monitoring provision at PDB.",
    requirement:
      "Ammeter or energy meter installed and functional.",
  },
  {
    activity_description:
      "Inspect padlocking provision for feeders (LOTO).",
    requirement:
      "Lockout/Tagout (LOTO) provision available and operable.",
  },
  {
    activity_description:
      "Verify feeders are provided with protective devices.",
    requirement:
      "Adequate overload and short-circuit protection installed.",
  },
  {
    activity_description:
      "Check isolation provision for control circuit.",
    requirement:
      "MCB or equivalent isolation device provided.",
  },
  {
    activity_description:
      "Inspect earthing provision for panel room equipment.",
    requirement:
      "Two-point earthing provided and securely connected.",
  },
  {
    activity_description:
      "Check panel door locking mechanism.",
    requirement:
      "Door lock functional and ensures restricted access.",
  },
];

function defaultPanelRoomChecklistRows(): ChecklistRow[] {
  return DEFAULT_PANEL_ROOM_CHECKLIST_SPEC.map((row, i) => ({
    localKey: nextKey(),
    sr_no: i + 1,
...
const createEmptyForm = (): SafetyPanelRoomFormState => ({
  ...
  audit_date: "",
  status: "draft",
  items: defaultPanelRoomChecklistRows(),
  documents: [],
  existingDocuments: [],
});
...
function recordToForm(
  ...
  return {
    ...
    isNew: false,
    isEditing: false,
    panel_name: asFormInputString(raw.panel_name),
    panel_type: normalizePanelType(raw.panel_type),
    location: asFormInputString(raw.location),
    audit_date: toDateInput(r.audit_date),
    status: (r.status as SafetyPanelRoomFormState["status"]) || "draft",
    items,
    documents: [],
...
/** Electrical safety — panel room checklist (`SafetyPanelRoomAudit`). */
export function SafetyPanelRoomSection({
  facilityId,
  utilityAccountId,
  auditStepLocked = false,
  auditStepNoData,
}: SafetyPanelRoomSectionProps) {
  ...
  const [forms, setForms] = useState<SafetyPanelRoomFormState[]>([]);
  const [backendError, setBackendError] = useState("");
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] =
    useState<SafetyPanelRoomFormState | null>(null);
  ...
  const updateForm = (
    localId: string,
    up: (f: SafetyPanelRoomFormState) => SafetyPanelRoomFormState,
  ) => {
  ...
  const handleCancel = (form: SafetyPanelRoomFormState) => {
    if (form.isNew) {
  ...
  const handleSave = async (form: SafetyPanelRoomFormState) => {
    setBackendError("");
    ...
    const payload = {
      ...
      panel_name: form.panel_name.trim() || undefined,
    ...
    try {
      await toastHandler({
        ...
        success: form.isNew
          ? "Panel room safety audit created"
          : "Saved",
      });
      await refetch();
    } catch (err) {
  ...
  const onDelete = (f: SafetyPanelRoomFormState) => {
    if (!f.id) return;
    setDeleteTarget(f);
    setDeleteOpen(true);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm text-destructive">
          Could not load panel room safety audits.{" "}
  ...
  if (listLoading) {
    return (
      <div className="text-sm text-muted-foreground">
        Loading panel room safety audits…
      </div>
    );
  }
          ...
          <Card key={form.localId}>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>
                ...
                {form.isNew ? " (new)" : ""}
                {typeof form.panel_name === "string" && form.panel_name
                  ? ` — ${form.panel_name}`
                  : ""}
              </CardTitle>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        panel_name: e.target.value,
                      }))
                    }
                    readOnly={!form.isEditing || auditStepLocked}
                    placeholder="e.g. LTP Basement PDB-01"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        panel_type: e.target.value as PanelType,
                      }))
                    }
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateForm(form.localId, (f) => ({
                        ...f,
                        status: e.target
                          .value as SafetyPanelRoomFormState["status"],
                      }))
                    }
                    disabled={!form.isEditing || auditStepLocked}
                  >
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this panel room safety audit. This
